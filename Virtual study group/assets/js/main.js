document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Page-Specific Logic ---
    if (pageIdentifier === 'dashboard' || pageIdentifier === 'dashboard.html') {
        populateDashboard();
    } else if (pageIdentifier === 'group' || pageIdentifier === 'group.html') {
        setupGroupDetailPage();
    } else if (pageIdentifier === 'groups' || pageIdentifier === 'groups.html') {
        setupGroupsPage();
    }
...
async function populateDashboard() {
    ...
    const API_URL = 'https://studysphere-backend-richa.onrender.com/api';
    try {
        // --- Sequential Fetching (THE FIX FOR REFRESH) ---
        ...
        const groupsRes = await fetch(`${API_URL}/groups/my`, { headers: { 'x-auth-token': token } });
        if (!groupsRes.ok) {
